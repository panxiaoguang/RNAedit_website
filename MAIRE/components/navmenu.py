...
class NavMenuContent(NavMenu):
    ...
    class_name: Var[str] | str = (
        "data-[motion=from-start]:animate-enterFromLeft data-[motion=from-end]:animate-enterFromRight data-[motion=to-start]:animate-exitToLeft data-[motion=to-end]:animate-exitToRight absolute top-4 left-0 w-[180px] border border-gray-200 bg-white rounded-md shadow-md p-2",
    )
...
class NavMenuViewport(NavMenu):
    ...
    class_name: Var[str] | str = (
        "data-[state=open]:animate-scaleIn data-[state=closed]:animate-scaleOut relative w-full transition-[width,_height] duration-300 bg-red-600"
    )
